...
namespace DxR
{
    ...
    public class Vis : MonoBehaviour
    {
        ...
        /// </summary>
        private void UpdateVis(string jsonString="")
        {
            DeleteAll();

            UpdateVisConfig();

            UpdateVisData(jsonString);
...
//                    throw new System.Exception("Make sure interaction object has type, field, and domain specs.");
        ...
        private void UpdateVisData(string jsonString="")
        {
            if (jsonString != "")
            {
                visSpecs["data"].Add("values", parser.CreateValuesSpecs("", jsonString));
            }
            else if (visSpecs["data"]["url"] != "inline")
            {
        ...
        public void UpdateVisSpecsFromJsonString(String jsonString)
        {

        }

        public void UpdateVisSpecsFromTextSpecs(string jsonString="")
        {
            // For now, just reset the vis specs to empty and
            // copy the contents of the text to vis specs; starting
            // everything from scratch. Later on, the new specs will have
            ...
            if (jsonString == "") gui.UpdateGUISpecsFromVisSpecs();
            UpdateVis(jsonString);
        }
